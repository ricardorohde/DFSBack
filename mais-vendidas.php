...
$iTM = new InterFaces(new Arquivos(Sistema::$layoutCaminhoDiretorio."mais-vendidas.html"));

$iTM->setSESSION($_SESSION);
$iTM->trocar('lang', $_SESSION['lang']);

//metatag
$titulo = $idioma->getTraducaoByConteudo("Mais Vendidas")->traducao;
$descricao = $idioma->getTraducaoByConteudo("Mais Vendidas")->traducao;
//
...
$aR[2] = array('campo' => ListaProdutos::PROMOCAO, 'valor' => ListaProdutos::VALOR_DISPONIVEL_TRUE);
	...
	while($p = $lP->listar()){
		...
		if($p->getImagens()->getTotal() > 0){
			...
			if($img->getImage()->nome != ''){
				$iTM->enterRepeticao()->trocar('imagem.Produto', $img->getImage()->showHTML(310, 418));
				$iTM->enterRepeticao()->trocar('url.Imagem.Produto', $img->getImage()->pathImage(310, 418));
			}
		}
